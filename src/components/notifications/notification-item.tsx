...
export function NotificationItem({ notification, onMarkRead }: NotificationItemProps) {
    ...
    return (
        <div className={cn(
            ...
            {/* Unread stripe */}
            {!read && (
                <span className="absolute left-0 top-3 bottom-3 w-0.5 rounded-full bg-[#A3E635]" />
            )}
            ...
            <div className="min-w-0 flex-1">
                <div className="flex flex-wrap items-start justify-between gap-x-3 gap-y-1">
                    <div className="flex flex-wrap items-center gap-2">
                        <p className={cn("text-sm font-semibold leading-snug", read ? "text-[#A1A1AA]" : "text-[#F4F4F5]")}>
                            {title}
                        </p>
                        <NotificationTypeBadge type={type} />
                        {!read && <span className="pulse-dot h-1.5 w-1.5 rounded-full bg-[#A3E635]" />}
                    </div>
                    <span className="shrink-0 text-[11px] text-[#A1A1AA]/40">{formatTimestamp(timestamp)}</span>
                </div>

                <p className={cn("mt-1 text-sm leading-relaxed", read ? "text-[#A1A1AA]/60" : "text-[#A1A1AA]")}>
                    {description}
                </p>
